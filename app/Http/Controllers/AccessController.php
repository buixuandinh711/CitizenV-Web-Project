    ...
    public function LoadDeclaredPermission() {
        if (session('user')) {
            if (session('user')->username == 'admin') {
                ...
                ->join('city', 'city.city_id', '=', 'access.username')
                ->selectRaw('city.city_name AS location')
                ->selectRaw('access.username AS user')
                ->selectRaw('access.start_date AS startDate')
                ->selectRaw('access.end_date AS endDate')->get();
                $result['info'] = $access;
                ...
                $access = DB::table('access')
                ->join('district', 'district.district_id', '=', 'access.username')
                ->selectRaw('district.district_name AS location')
                ->selectRaw('access.username AS user')
                ->selectRaw('access.start_date AS startDate')
                ->selectRaw('access.end_date AS endDate')
                ...
                $access = DB::table('access')
                ->join('ward', 'ward.ward_id', '=', 'access.username')
                ->selectRaw('ward.ward_name AS location')
                ->selectRaw('access.username AS user')
                ->selectRaw('access.start_date AS startDate')
                ->selectRaw('access.end_date AS endDate')
                ...
                $access = DB::table('access')
                ->join('village', 'village.village_id', '=', 'access.username')
                ->selectRaw('village.village_name AS location')
                ->selectRaw('access.username AS user')
                ->selectRaw('access.start_date AS startDate')
                ->selectRaw('access.end_date AS endDate')
        ...
        if (session('user')) {
            if (session('user')->username == 'admin') {
                ...
                if ($username == "" || $start_date == "" || $end_date == "" || strlen($username) != 8 || count($access) || 
                    !ctype_digit($username) || !count($access_city) || !count($access_district) || !count($access_ward)) {
                    $result['resp'] = 'error';
                    return response()->json($result);
                }
                DB::table('access')->insert(['username' => $username,'start_date' => $start_date, 'end_date' => $end_date]);
    ...
    public function DeletePermission(Request $request) {
        if (session('user')) {
            if (session('user')->username == 'admin') {
                ...
                $username = session('user')->username.$request->code;
                $access = DB::table('access')->where('username', $username)->get();
                if ($username == "" || strlen($username) != 6 || !count($access) || !ctype_digit($username) || !count($access_city) || 
                ...
                $username = session('user')->username.$request->code;
                $access = DB::table('access')->where('username', $username)->get();
                if ($username == "" || strlen($username) != 8 || !count($access) || !ctype_digit($username) || !count($access_city) || 
    ...
    public function DeclarePermissionLocationInfo() {
        if (session('user')) {
            if (session('user')->username == 'admin') {
                ...
                $noaccesslocation = DB::table('city')
                ->leftjoin('access', 'city.city_id', '=', 'access.username')
                ->selectraw('city.city_id as code')
                ->selectraw('city.city_name as name')
                ->whereraw('access.username is null')->get();
                ...
                $noaccesslocation = DB::table('district')
                ->leftjoin('access', 'district.district_id', '=', 'access.username')
                ->selectraw('district.district_id as code')
                ->selectraw('district.district_name as name')
                ->whereraw('access.username is null')
                ->where('city_id',session('user')->username)->get();
                $result['nonGrantedLocation'] = $noaccesslocation;
                ...
                $noaccesslocation = DB::table('ward')
                ->leftjoin('access', 'ward.ward_id', '=', 'access.username')
                ...
                $noaccesslocation = DB::table('village')
                ->leftjoin('access', 'village.village_id', '=', 'access.username')
